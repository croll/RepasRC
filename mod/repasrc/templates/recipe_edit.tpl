			...
			<div class="well" style="padding: 8px 0;">
				<ul class="nav nav-list">
					...
					{if isset($recipe_id)}
						...
						{if (isset($smarty.session.recipe.modules) && $smarty.session.recipe.modules.transport == 1)}<li{if ($section == 'transport')} class="active"{/if}><a href="/recette/analyse/transport/{$recipe_id}"><i class="icon-road"></i>{t d='repasrc' m='Transport'}</a></li>{/if}
						{if (isset($smarty.session.recipe.modules) && $smarty.session.recipe.modules.prodcons == 1)}<li{if ($section == 'mode')} class="active"{/if}><a href="/recette/analyse/mode/{$recipe_id}"><i class="icon-map-marker"></i>{t d='repasrc' m='Production / conservation'}</a></li>{/if}
						{if (isset($smarty.session.recipe.modules) && $smarty.session.recipe.modules.price == 1)}<li{if ($section == 'prix')} class="active"{/if}><a href="/recette/analyse/prix/{$recipe_id}"><i class="icon-barcode"></i>{t d='repasrc' m='Prix'}</a></li>{/if}
						<li{if ($section == 'comparer')} class="active"{/if}><a href="/recette/analyse/comparer/{$recipe_id}"><i class="icon-retweet"></i>{t d='repasrc' m='Comparer'}</a></li>
					{/if}
				</ul>
			</div>
			{/block}
